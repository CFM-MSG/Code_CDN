...
def soft_ms(dets, thresh=0.4, top_k=-1):
    """Pure Python NMS baseline."""
    if len(dets) == 0: return []
    order = np.arange(0, len(dets), 1)
    dets = np.array(dets)
...
def eval(segments, data):
    # print('len seg:', len(segments), '  len data:',len(data))
    ...
    for i in range(len(recalls)):
        for j in range(len(tious)):
            writer.add_scalar('{}/ACC/Rank@{}/mIoU@{}'.format(split, recalls[i], tious[j]),
                              eval_result[j][i],
                              iter)
            # writer.add_scalars('ACC', {'{},Rank@{},mIoU@{}'.format(split, recalls[i], tious[j]): eval_result[j][i]},
            #                    iter)

    writer.add_scalar('{}/MIOU'.format(split), miou, iter)
    # writer.add_scalars('MIOU', {split: miou}, iter)
