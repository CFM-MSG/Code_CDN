...
def get_padded_mask_and_weight(*args):
    ...
    masked_weight[masked_weight > 0] = (1 / masked_weight[masked_weight > 0]).to(masked_weight.dtype)  # conv.kernel_size[0] * conv.kernel_size[1]
    padded_mask = masked_weight > 0

    return padded_mask, masked_weight


from .map_conv import MapConv, DynamicMapResConv