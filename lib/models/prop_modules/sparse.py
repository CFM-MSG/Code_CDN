...
class SparsePropMaxPool(nn.Module):
    ...
    def forward(self, x):  # batchsize * 512 * 16
        map_h_list = []
        map_mask_list = []

        for scale_idx, scale_layers in enumerate(self.layers):
            batch_size, hidden_size, num_scale_clips = x.shape
            num_scale_clips = num_scale_clips // scale_layers[0].stride  # 16
            map_h = x.new_zeros(batch_size, hidden_size, num_scale_clips, num_scale_clips)  # batchsize * 512 * 16 *16
            map_mask = x.new_zeros(batch_size, 1, num_scale_clips, num_scale_clips)  # batchsize * 1 * 16 *16
    ...
    def recover_to_original_map(self, h_list, mask_list):
        ...
        batch_size, hidden_size, ori_num_clips, _ = h_list[0].shape  # batchsize * 512 * 16 *16

        ori_map_h = h_list[0].new_zeros(batch_size, hidden_size, ori_num_clips,
                                        ori_num_clips)  # batchsize * 512 * 16 *16
        ori_map_mask = mask_list[0].new_zeros(batch_size, 1, ori_num_clips, ori_num_clips)  # batchsize * 1 * 16 *16
        acum_layers = 0
        stride = 1
        for scale_layers, h, mask in zip(self.layers, h_list, mask_list):
            num_scale_clips = h.shape[-1]  # 16
            for i, layer in enumerate(scale_layers):
...
class SparsePropConv(nn.Module):
    ...
    def forward(self, x):
        ...
        for scale_idx, scale_layers in enumerate(self.layers):
            batch_size, hidden_size, num_scale_clips = x.shape  # batchsize * 512 * 16
